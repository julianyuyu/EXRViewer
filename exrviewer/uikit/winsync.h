...
public:
	AVRAtomicInt() : ival(0) {}
	AVRAtomicInt(int v) : ival(v) { }
	int ival;
	inline int operator=(int c) const
	{
		return InterlockedExchange((unsigned int *)&ival, c);
	}

	inline int operator+(int c) const
	{
		return InterlockedExchangeAdd((unsigned int *)&ival, c);
	}
	...
	inline int operator=(AVRAtomicInt& c) const
	{
		return InterlockedExchange((unsigned int *)&ival, c.ival);
	}

	inline int operator+(AVRAtomicInt& c) const
	{
		return InterlockedExchangeAdd((unsigned int *)&ival, c.ival);
	}
	...
	{
		return InterlockedIncrement((unsigned int *)&ival);
	}
	inline int operator--() const
	{
		return InterlockedDecrement((unsigned int *)&ival);
	}
};
...
typedef DWORD(WINAPI *PFN_WINTHREAD_PROC)(LPVOID param);

class WinThread
{
public:
	WinThread() : m_hThread(NULL), m_ThreadId(0), m_bSuspended(false) {}
	WinThread(PFN_WINTHREAD_PROC proc, LPVOID lpParameter, bool bCreateSuspended = false) :
		m_hThread(NULL), m_ThreadId(0), m_bSuspended(false)
	{
		Create(proc, lpParameter, bCreateSuspended);
	}
	
	~WinThread() { if (m_hThread) { CloseHandle(m_hThread); m_hThread = NULL; } }
	HANDLE Create(PFN_WINTHREAD_PROC proc, LPVOID lpParameter, bool bCreateSuspended = false)
	{
